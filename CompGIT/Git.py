...
def proportional(v1, v2) -> bool:
    """
    Decide if two non-zero vectors are proportional.

    The two vectors can be of any class that allows iteration and
...
class GITProblem(object):
    ...
    def intersection_set(self, I_i, ray_i, monomial_0) -> Set:
        """
    ...
    def compute_weights_in_all_unstable_states(self):
        """
        Compute the Set of weights that are present in all unstable states and stores it in the object.

        In most cases this set may be empty.
    ...
    def solve_non_stable(self, Weyl_optimisation=False):
        ...
        - ``Weyl_optimisation`` -- If ``True`` it will only consider
          one-parameter subgroups within the fundamental chamber rather
          than the whole lattice, potentially reducing the final output
          by eliminating isomorphic unstable/non-stable families.

        Note that as of January 2025, it is unknown whether this
        really reduces the number of maximal families (see Conjecture
        7.4 in [GMGMS]) as the output is the same with or without Weyl
        optimisation.
    ...
    def solve_unstable(self, Weyl_optimisation=False):
        ...
        Note that as of January 2025, it is unknown whether this
        really reduces the number of maximal families (see Conjecture
        7.4 in [GMGMS]_) as the output is the same with or without Weyl
        optimisation.
